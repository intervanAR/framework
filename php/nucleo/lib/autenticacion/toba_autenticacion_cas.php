...
class toba_autenticacion_cas extends toba_autenticacion implements toba_autenticable
{
	...
	function __construct()
	{
		...
		if (is_file( $archivo_ini_instalacion)) {
			$parametros = toba::config()->get_subseccion('idp','cas');
	...
	protected function iniciar_pedido_cas()
	{
		...
		/** Llamada principal al authentificaci�n de CAS, si no est�s
		autenticado te redirecciona ah� adentro y no sigue ejecutando
		Si pasa est� funci�n significa que est�s autenticado **/
