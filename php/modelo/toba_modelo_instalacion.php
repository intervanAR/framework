...
class toba_modelo_instalacion extends toba_modelo_elemento
{
	...
	*/
	...
	/**
	* Retorna true si la instalaci�n es de producci�n (implementaci�n)
	*/
	...
	*/
	...
	/**
	 * Cambia o agrega algunos parametros al archivo de informaci�n de la instalaci�n
	...
	*/
	...
	/**
	 * Toma un proyecto de una instancia de un toba en un versi�n anterior e importa el mismo a esta versi�n
	 * La instancia origen se debe llamar igual que la destino
	 * @param string $id_instancia Instancia origen/destino de la migraci�n
	 * @param string $id_proyecto Proyecto origen/destino de la migraci�n
	...
	function importar_migrar_proyecto($id_instancia, $id_proyecto, $dir_toba_viejo, $destino=null)
	{
		...
		try {
			...
			if (! isset($bases_viejas[$id_base_instancia])) {
				throw new toba_error("No se encuentra la definici�n de la instancia $id_base_instancia en el archivo $archivo_ini_bases");
			} 

			//--- Incluir solo el proyecto a importar en la instancia
			$this->manejador_interface->titulo("2.- Apuntando la instancia nueva a la de la versi�n anterior");
			...
			$this->manejador_interface->titulo("3.- Migrando el proyecto de versi�n toba");
			...
			if (file_exists($dir_backup)) {
				...
				throw new toba_error('Imposible restaurar el estado previo a la migraci�n');
			}
			...
			if (file_exists($dir_backup)) {
				...
				if (file_exists($dir_original)) {
					if (! toba_manejador_archivos::eliminar_directorio($dir_original)) {
						throw new toba_error("Imposible restaurar backup desde '$dir_backup' hacia '$dir_original', deber� hacerlo manualmente.");
					}
				}
				rename($dir_backup, $dir_original);
			}
	...
	{
		toba_logger::instancia()->debug("Migrando instalaci�n hacia version ".$version->__toString());
		$this->manejador_interface->mensaje("Migrando instalaci�n.", false);
	...
	private static function actualizar_version($version)
	{
		...
		toba_logger::instancia()->debug("Actualizada instalaci�n a versi�n $numero");
	}
